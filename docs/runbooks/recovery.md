...
## Manual cleanup for stuck `in_progress` sessions

Use this when `practice_sessions` remain in `in_progress` for more than 15 minutes.
...
left join public.session_attempts sa on sa.practice_session_id = ps.id
...
set
  status = 'failed',
  failure_reason_code = 'in_progress_stuck_timeout',
...
set
  ...
  updated_at = timezone('utc', now())
from public.practice_sessions ps
where sa.practice_session_id = ps.id
  and ps.status = 'failed'
  and ps.failure_reason_code = 'in_progress_stuck_timeout'
